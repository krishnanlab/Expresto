...
for aGPL in GPLs:
    ...
    df_eval = add_eval_to_df(df_eval, mae_values,'mae','GeneKNN',col_names,aGPL)
    ...
    df_eval = add_eval_to_df(df_eval, cvrmse_values,'cvrmse','GeneKNN',col_names,aGPL)
    # save the dataframe
    df_eval.to_csv(fp_save + '%s_GeneKNN_evals.tsv'%aGPL,sep='\t',header=True,index=False)
