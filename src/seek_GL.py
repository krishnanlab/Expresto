...
# do the machine learning for gene lasso
print('Doing ML for GeneLasso')
preds = np.zeros((ytst.shape[0],ytst.shape[1]),dtype=float)
for idx in range(ytrn.shape[1]):
    tic1 = time.time()
    clf = Lasso(alpha=0.001,fit_intercept=True,normalize=False,precompute=False,copy_X=True,max_iter=1000,tol=0.001,
...
df_eval = add_eval_to_df(df_eval, mae_values,'mae','GeneLASSO',col_names,aGPL)
...
df_eval = add_eval_to_df(df_eval, cvrmse_values,'cvrmse','GeneLASSO',col_names,aGPL)
# save the dataframe
df_eval.to_csv(fp_save + '%s_GeneLASSO_evals.tsv'%aGPL,sep='\t',header=True,index=False)
